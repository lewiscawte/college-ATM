...
void transferFunds() {
	...
	// Variables to match the globals, renamed to victim for recipient. 
	char victimPin[5]="", victimCustName[41]="", victimCardNumb[17]="";
	float victimBalance=0, transferAmount=0;

	// @term: victim - the person who is the recipient
	...
	printf("Transferring %.2f to %s (Acct. %s)", transferAmount, victimCustName, victimCardNumb).
}